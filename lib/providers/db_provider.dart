  ...
  Future<List<Informe>> getActividadAgrupadaPorMes() async {
    try {
      ...
      var resultado = await db.rawQuery(
          "SELECT STRFTIME('%m-%Y', fecha) AS mes, SUM(minutosTotales) AS minutosTotales FROM informe GROUP BY STRFTIME('%m-%Y', fecha) ORDER BY mes");
      // "SELECT STRFTIME('%m-%Y', fecha) AS mes, SUM(minutosTotales) as minutosTotales FROM informe GROUP BY minutosTotales");
      return resultado
          .map((e) => Informe(
                // id: 1,
